...
this is always 4 (1 opcode + 3 parameters)
...
to produce the output 19690720. What is 100 * noun + verb?
(For example, if noun=12 and verb=2, the answer would be 1202.)
'''


with open("day02_input.txt", 'r') as f:
    codes = [int(value) for value in f.read().split(',')]


def execute_program(codes, verb, noun):
    codes[1], codes[2] = verb, noun
    for i in range(0, len(codes), 4):
        code = codes[i]
        if code == 99:
            break
        firstIndex = codes[i + 1]
        secondIndex = codes[i + 2]
        rewriteIndex = codes[i + 3]
        if code == 1:
            codes[rewriteIndex] = codes[firstIndex] + codes[secondIndex]
        elif code == 2:
            codes[rewriteIndex] = codes[firstIndex] * codes[secondIndex]
    return {'output': codes[0], 'verb': codes[1], 'noun': codes[2]}
...
def find_inputs(n):
    for i in range(n, n * 10):
        for j in range(n, n * 10):
            verb, noun = i, j
            try:
                res = execute_program(codes.copy(), verb, noun)
                if res['output'] == 19690720:
                    print(f"Part 2 Answer: {100 * res['verb'] + res['noun']}")
                    return True
